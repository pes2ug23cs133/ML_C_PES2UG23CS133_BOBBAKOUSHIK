...
# This file contains the complete, self-contained solution for the Mushroom dataset.
# It includes all necessary functions for data processing, tree construction, and evaluation.

# == Functions from lab_Sample_Solution.py ==
...
def get_entropy_of_dataset(data):
    ...
    Args:
        data: A PyTorch tensor or NumPy array representing the dataset.
    ...
    unique_values, counts = torch.unique(target_column, return_counts=True)
...
def get_avg_info_of_attribute(data, attribute: int):
    """
    Return the average information of the specified attribute.

    Args:
        data: A PyTorch tensor or NumPy array representing the dataset.
        attribute: The index of the attribute column.

    Returns:
        The average information of the attribute as a float.
    """
    if isinstance(data, np.ndarray):
        data = torch.from_numpy(data)

    attribute_column = data[:, attribute]
    unique_attribute_values = torch.unique(attribute_column)

    avg_info = 0.0
    total_samples = len(data)

    for value in unique_attribute_values:
        # Create a mask to get the subset of data for this attribute value
...
def get_information_gain(data, attribute: int):
    """
    Return the information gain of the specified attribute.
    ...
    Returns:
        The information gain of the attribute as a float.
    """
    if isinstance(data, np.ndarray):
        data = torch.from_numpy(data)

    entropy_of_dataset = get_entropy_of_dataset(data)
    avg_info = get_avg_info_of_attribute(data, attribute)
    
    return entropy_of_dataset - avg_info
...
def get_selected_attribute(data):
    ...
    Returns:
        A tuple containing:
        - A dictionary with information gain of all attributes.
        - The index of the selected attribute with the highest gain.
    """
    if isinstance(data, np.ndarray):
        data = torch.from_numpy(data)
    ...
    return (information_gains, int(selected_attribute))
...
def calculate_tree_complexity_metrics(tree):
    """Calculates tree complexity metrics like depth and number of nodes."""
...
def evaluate_decision_tree(tree, X_test, y_test, cols, class_names=None):
    """Evaluates the decision tree on a test set and prints metrics."""
    predictions = []
...
def convert_data_for_framework(data, target_framework):
    """Converts data to the specified framework format."""
...
def get_unique_values(data, column_idx, target_framework):
    """Get unique values in a framework-agnostic way."""
    if target_framework == 'pytorch':
        if isinstance(data, torch.Tensor):
...
def create_mask(data, column_idx, value, target_framework):
    """Create a boolean mask in a framework-agnostic way."""
    if target_framework == 'pytorch':
        if isinstance(data, torch.Tensor):
...
def get_item_value(value, target_framework):
    """Extract a scalar value in a framework-agnostic way."""
    if target_framework == 'pytorch':
...
def preprocess_data(df):
    """Preprocesses the dataset by encoding categorical variables."""
...
# == Main Execution Block (equivalent to test.py) ==

def test_case(data_path, framework='pytorch', print_tree_flag=False, print_construction_flag=False):
    ...
    try:
        ...
        if tree is not None:
            ...
            evaluate_decision_tree(tree, X_test, y_test, cols, class_names)
        else:
...
if __name__ == "__main__":
    test_case(data_path=r"C:\Users\bobba\Downloads\all\tictactoe.csv")